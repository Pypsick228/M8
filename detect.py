...
def detect_trash(image):
    ...
    if trash == "Plastic bottle":
        return "Бутылки из пластика следует выбрасывать в специальные мусорные баки ,пластик из которых в скором времени попадает на переработку.В 2015 году было проведено исследование ,в котором выяснилось ,что только 20 процентов пластика перерабатывается.К сожалению большинство пластиковых отходов пригодны только для одного цикла переработки ,ибо процесс переработки ухудшиет общую целостность пластика."
    if trash == "Glass bottle":
        return "Переработкой стекла занимаются стеклоперерабатывающие заводы. В отличие от переработки пластика, эта система налажена и успешно работает уже долгие годы."
